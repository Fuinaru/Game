...
Shader "Unity Shaders Book/Chapter14/Perfect" {
	Properties{
...
#include "UnityCG.cginc"
	...
	v2f vert(a2v v) {
		...
		float3 mDir = normalize(v.vertex.xyz);
		mDir=mul((float3x3)UNITY_MATRIX_IT_MV, mDir);
		float3 d;
		float factor = dot(mDir, normal);
		//if(dot(mDir, normal)>=0)d = lerp(mDir,normal, factor*_fix+1);
		//else  d = lerp(-mDir, normal, -factor*_fix+1);
		d= lerp((step(0, factor) * 2 - 1)*mDir, normal, (step(0,factor)*2-1)*factor*_fix + 1);
		d.z = -0.1;
		pos = pos + float4(normalize(d), 0)*_Outline;
